...
public class AduitFragment extends BaseFragment {
    ...
    protected void initData() {
        ...
        madpater.setListonclicklister(new ListOnclickLister() {
            ...
            public void onclick(View v, int position) {
                choicedata = mListdata.get(position);
    ...
    private void getdata(){
        ...
        Observable observable =
                ApiUtils.getApi().tasklistdata(new TasklistdataBody(BaseActivity.getuser().UserId,"我发布未审核"))
...
//                ToastUtils.makeText(message);
    ...
    private void updateAudit(int index){
        Observable observable =
                ApiUtils.getApi().aduditdata(new AuditBody(BaseActivity.getuser().UserId,choicedata.Feedbackid,index==1?"1":index==2?"4":"4"))
                        .compose(RxHelper.getObservaleTransformer())
                        .doOnSubscribe(new Consumer<Disposable>() {
                            @Override
                            public void accept(Disposable disposable) throws Exception {
