...
public abstract class BaseActivity extends AppCompatActivity {
    ...
    public static int mcode = -1;
    public static UserdataBean userdata = null;
    @Override
    ...
    public static UserdataBean getuser(){
        try {
            if (userdata == null) {
                String loginuser = PreferencesUtils.getInstance().getString(Contans.USER_DATA, "");
                if (TextUtils.isEmpty(loginuser)) {
                    return null;
                } else {
                    userdata = new Gson().fromJson(loginuser,UserdataBean.class);
                    if (userdata != null) {
                        return userdata;
                    } else {
                        return null;
